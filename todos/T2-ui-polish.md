...
> **状态**：未开始（**M5 交付后可做**；**建议 T1 路由更名完成后再精修**，避免 `/ai`→`/chat` 返工）  
> **周期**：约 2～4 人周（1～2 人）  
...
M0～M5 / M3 签收标准是 **行为 / API / 存储键 parity**，**不要求像素复刻**（同 MCP-Client T3/T5）。迁移采用 React + Next 重写，存在典型视觉债：
...
本任务书用于 **M5 之后集中偿还视觉债**；与 T1 命名债并列，均属 post-parity 优化。
...
> 可在 M5 后做 **T2-00**（录基准、记 backlog），**不写**精修代码。
...
| M5 完成前大范围改 UI | 与 parity 冲突；仅允许修 **阻断使用** 的 UI bug（记入对应 Mx 批次） |
...
```
M0～M5（parity，行为交付）→ MS5
  → T1（路由/文案，可选但建议在 T2 前）
  → T2（UI 精修）
  → （可选）API 路由收敛
```

**禁止**在 M3～M5 功能迁移 PR 中夹带 T2 视觉大改（阻断级小修除外）。
