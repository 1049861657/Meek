...
| `src/mcp-servers/` | `apps/worker/mcp-servers/`（stdio 子进程，worker 内嵌 `tsc` → `dist/mcp-servers/`） |
| `src/config-plane/` | `packages/config-plane` |
| `src/message-bus/`、`src/channels/` | `apps/worker` |
| `prisma/schema.prisma` | `packages/db`（14 model） |

页面：`index→/`、`chat→/chat`、`settings→/settings`、`mcp→/mcp`、`admin→/admin`。
...
M2+M4+M1 → M5 IM 渠道 + Bus 完善 → T1 命名债 → T2 UI 精修（可选）
```
...
- **MS5**：M0～M5 迁移主体交付（parity 功能已落地；联调问题按需修）
...
| [T1-routing-naming.md](./T1-routing-naming.md) | **M5 后** 路由/文案命名债偿还（非 parity） |
| [T2-ui-polish.md](./T2-ui-polish.md) | **M5 后** Web UI 视觉精修（建议 T1 后；可选 Pencil） |
...
**当前：T1 路由/命名债已偿还 — 可选 T2 UI 精修，问题按需修**
